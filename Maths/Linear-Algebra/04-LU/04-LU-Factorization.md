...
**The Math:** $A$ is $m \times n$. $L$ is lower triangular with 1s on the diagonal; the multiplier $l_{ij}$ used to eliminate entry $(i,j)$ goes directly into position $(i,j)$ of $L$. $U$ is upper triangular with pivots on the diagonal. When row exchanges are needed, $PA = LU$ where $P$ is a 05-Transposes-Permutations-Spaces recording the swaps. The factorization requires all pivots to be non-zero (or made non-zero by permutation). You can factor out the pivots into a diagonal matrix $D$ to get $A = LDU$, where $L$ and $U$ both have 1s on the diagonal. The cost of elimination is $n^3/3$ operations; once $L$ and $U$ are computed, solving for any new $\mathbf{b}$ costs only $n^2$ operations — a massive savings for multiple right-hand sides.
