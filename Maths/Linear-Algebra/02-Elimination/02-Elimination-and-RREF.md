...
We know what $A\mathbf{x} = \mathbf{b}$ means — but how do you actually solve it? Gaussian elimination is the fundamental algorithm: reduce any matrix to upper triangular form $U$ by forward elimination, then to reduced row echelon form $R$ by back-substitution. The structure it reveals — pivots, free variables, special solutions — determines everything about the solution set. This is the engine behind 04-LU-Factorization and the entry point to understanding 06-Complete-Solutions-and-Rank.
